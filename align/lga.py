...
LGATaskParams = {
    ...
    "is_first_task": True,
    "frame_interval_thorough_check": 10,
    "frame_interval_continuous_check": 4,
    "fixed_width": -1,
    "iou_thresh": 0.58,
}
...
def LGATask(desc: LGATaskDescriptor):
    ...
    # gocator.visualize(dfs, f"{desc.id}_gocator_original.png")

    (
        aligned_depths,
        crop_start,
        crop_end,
    ) = gocator.get_projected_depth_map_to_line_scan(line_scan, dfs, cropped=False)

    aligned_image = GoCator.get_image_from_depths(aligned_depths)
    # line_scan.visualize(aligned_image, f"{desc.id}_gocator_project.png")
    ...
    np.savetxt(
        os.path.join(
            common.output_path,
            f"{desc.id}_gocator_reprojected.txt",
        ),
        aligned_depths,
        fmt="%.5f",
    )
